...
fn group_by_size(files: Vec<ParquetFile>, max_file_size_bytes: u64) -> Vec<Vec<ParquetFile>> {
...
mod tests {
    ...
    async fn test_compact_remaining_level_0_files_one_level_0_without_overlap() {
        ...
        compact_remaining_level_0_files(&compactor, Arc::clone(&c))
            .await
            .unwrap();
        ...
        full_compaction(&compactor, c).await.unwrap();
